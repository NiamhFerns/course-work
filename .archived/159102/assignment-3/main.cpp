...
vector<button> buttons; // These are the globals from the starting code just... usable and not as annoying.
...
// Given a list of cordinates for a button (maxX, maxY, minX minY), find a radius, the centre point,
// and then use these to determine whether the button is perfectly circular or missing bits by
// checking whether radius at angle theta is still a pixel with an R/G/B value > 128.
...
void findDamaged() {
    ...
    // but yeah. c:
    //
    // I'd really have liked to find a way to check the edge pixels of the button against their distance from the centre to determine
    // whether they are not a button and thus are damaged, but I couldn't figure out how.
    // So, I just drew a button with a maxDeviation distance from the edge and checked each pixel on that button. I'd really like feedback
    ...
    for (int i = 0; i < buttons.size(); ++i) {
        ...
        // We need the radius to be the max() of either height or width - the maxDeviation.
        // That is... we need the radius around the button at which we would consider any pixels < 128 to be past the maximum allowed
        // varience for a button. It also accounts for if the difference of the height and width being greater than maxDeviation.
        // We use an outer radius to check if it's circular, then an inner radius alongside findArea() to check whether there are 4 holes.
