...
public class Lecturer {
    ...
    public static void printOfferings(int id, ArrayList<Course> courses) {
        for (Course c : courses) {
            for (int i = 0; i < 3; ++i) {
                if (c.getLecturers()[i].staffID == id) {
                    System.out.println("Paper Offering -  " + c.getCode() + "    " + locations[i] + "   Lecturer: " + c.getLecturers()[i].toString());
                }
            }
        }
    }
    ...
    private static Lecturer findLecturerFor(String campus, Lecturer[] staff) {
        ...
        if (campus.equals("Distance")) {
            for (int i = 0; i < staff.length; ++i) {
                if (staff[i].distanceTeacher) continue;
                staff[i].distanceTeacher = true;
                staff[i].currentlyTeaching++;
                return staff[i];
            }
        }
